...
public class RecipeImageFragment extends NavigableFragment {
    ...
    public static RecipeImageFragment newInstance(Receita recipe) {
    ...
    public void onNext() {
        if (currentRecipeImage == null) {
            Toast.makeText(getActivity(), "Escolha uma imagem para essa receita.", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        if (name.isEmpty()) {
            Toast.makeText(getActivity(), "Nome da receita é obrigatório.", Toast.LENGTH_LONG).show();
            return;
        }

        if (description.isEmpty()) {
            Toast.makeText(getActivity(), "Por favor, insira a descrição.", Toast.LENGTH_LONG).show();
            return;
        } else {
            if (description.length() > 200) {
                Toast.makeText(getActivity(), "Sua descrição não deve ter mais que " + MAX_DESCRIPTION_LENGTH + " caracteres.", Toast.LENGTH_LONG).show();
                return;
            }
        }
    ...
    public void onImageSelected(String imagePath) {
        ...
        if (!currentRecipeImage.isEmpty()) {
            ...
            selectImageBtn.setText("Atualizar imagem da receita");
        }
    }
